...
public class SimSwerveDriveTest {
  ...
  @Test public void meme() {
    var pos = new Translation2d(1 / Math.sqrt(2), 1 / Math.sqrt(2));
    var force = new Translation2d(-1 / Math.sqrt(2), 1 / Math.sqrt(2));
    var moduleTorque = pos.getX() * force.getY() - pos.getY() * force.getX();
    assertEquals(1, moduleTorque, 1e-4);
  }

  @Test
  public void testSimulationDrivingForward() {
//    System.out.println("FL angle, FL speed, FR angle, FR speed, BL angle, BL speed, BR angle, BR speed, vx, vy, omega");

    // v = r = sqrt(2)

    var desiredSpeeds =
        m_sim.getKinematics().toSwerveModuleStates(new ChassisSpeeds(0, 0, 1));

    var chassisSpeedList = new ArrayList<ChassisSpeeds>();

    for (int i = 0; i < 50; i++) {
      m_sim.setModuleVoltages(2, 2, 2, 2);
