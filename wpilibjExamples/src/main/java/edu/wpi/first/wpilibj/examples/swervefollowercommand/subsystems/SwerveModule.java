...
import static edu.wpi.first.wpilibj.examples.swervefollowercommand.Constants.ModuleConstants.*;
...
public class SwerveModule {
   ...
   */
  public SwerveModule(int driveMotorChannel, int turningMotorChannel, int[] driveEncoderPorts, int[] turningEncoderPorts, boolean driveEncoderReversed, boolean turningEncoderReversed) {
    ...
    // resolution.
    m_driveEncoder.setDistancePerPulse(kEncoderDistancePerPulse);

    //Set whether encoder should be reversed or not
    m_driveEncoder.setReverseDirection(driveEncoderReversed);

    // Set the distance (in this case, angle) per pulse for the turning encoder.
    // This is the the angle through an entire rotation (2 * wpi::math::pi)
    // divided by the encoder resolution.
    m_turningEncoder.setDistancePerPulse(kEncoderDistancePerPulse);
